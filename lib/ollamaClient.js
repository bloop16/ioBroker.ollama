 ...
 * Now includes intelligent model queue management to prevent conflicts
 */
class OllamaClient {
  /**
  ...
  constructor(openWebUIUrl, logger, config, translateFn) {
    ...
    this._ollamaUrl =
      config?.ollamaUrl ||
      OllamaClient.createHttpUrl(
        config?.ollamaIp || "127.0.0.1",
        config?.ollamaPort || 11434,
      );
    this._toolServerUrl =
      config?.toolServerUrl || OllamaClient.createHttpUrl("127.0.0.1", 9100);
    ...
    // Initialize ModelManager for intelligent request queuing
    this.modelManager = new ModelManager(
      this._ollamaUrl,
      logger,
      this._httpClient,
    );
    this.modelManager.setRequestHandler(this._executeModelRequest.bind(this));

    const logMessage = this.translate("general_initialized").replace(
      "{{component}}",
      "OllamaClient (OpenWebUI-first with Queue Management)",
    );
  ...
  /**
   * Execute a model request through the appropriate backend
   * This is called by the ModelManager when a request is ready to execute
   *
   * @param {string} modelName - Name of the model
   * @param {object} requestData - Request data
   * @returns {Promise<string>} Model response
   */
  async _executeModelRequest(modelName, requestData) {
    ...
    return await this._processChatViaOllama(
      modelName,
      requestData.messageObj,
      requestData.options,
    );
   ...
   * Now uses ModelManager for enhanced status tracking
   */
  async checkRunningModels() {
    const status = this.modelManager.getModelStatus();

    // Return format compatible with existing monitoring system
    return status.loaded.map((name) => ({ name, model: name }));
  }

  /**
   * Process chat message with intelligent queuing and conflict prevention
   * Uses ModelManager to queue requests and prevent model conflicts
   *
  ...
  async processChatMessage(modelName, messageObj, options = {}) {
    ...
    this.log.debug(
      `[OllamaClient] Queuing chat message for model ${modelName}`,
    );

    // Check queue status
    const queueLength = this.modelManager.getQueueLength(modelName);
    const isAvailable = this.modelManager.isModelAvailable(modelName);

    if (!isAvailable) {
      this.log.info(
        `[OllamaClient] Model ${modelName} is busy, queuing request (queue length: ${queueLength})`,
      );
    }

    // Use ModelManager to queue the request
    const requestData = {
      messageObj,
      options,
    };

    return await this.modelManager.queueRequest(modelName, requestData);
  }
  ...
  /**
   * Create HTTP URL from IP and port
   *
   * @param {string} ip - IP address
   * @param {number|string} port - Port number
   * @returns {string} Complete HTTP URL
   */
  static createHttpUrl(ip, port) {
    return `http://${ip}:${port}`;
  }
}
