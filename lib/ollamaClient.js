...
class OllamaClient {
    ...
    constructor(openWebUIUrl, logger, config, translateFn) {
        ...
        this._ollamaUrl = config?.ollamaUrl || `http://${config?.ollamaIp || '127.0.0.1'}:${config?.ollamaPort || 11434}`;
        ...
        const logMessage = this.translate('general_initialized').replace('{{component}}', 'OllamaClient');
    ...
    setToolServerUrl(toolServerUrl) {
        ...
        if (this.log.debug) {
            this.log.debug(this.translate('server_updated').replace('{{url}}', toolServerUrl));
        } else {
            this.log.info(this.translate('server_updated').replace('{{url}}', toolServerUrl));
        }
    }
    ...
    configureDatapointControl(enabled, allowedDatapoints, adapter = null) {
        ...
        if (enabled) {
            ...
            const mockAdapter = adapter || {
                getForeignStateAsync: async (id) => ({ val: 'test', ts: Date.now() }),
                getObjectAsync: async (id) => ({ common: { type: 'boolean' } }),
                setStateAsync: async (id, val) => true
            };
    ...
    async fetchModels() {
        try {
            this.log.debug(`[API] Fetching models from Ollama: ${this._ollamaUrl}/api/tags`);
            ...
            if (resp.data && resp.data.models) {
                ...
                this.log.info(`[Ollama] Successfully fetched ${models.length} models: ${models.join(', ')}`);
                return models;
            }
            ...
            if (error.response) {
                this.log.error(`[Ollama] HTTP Error ${error.response.status}: ${error.response.statusText || 'Unknown'}`);
        ...
        try {
            ...
            if (options.options && typeof options.options === 'string' && options.options !== "{}") {
                try {
                    ...
                    this.log.warn(`[API] Failed to parse custom options: ${error.message}`);
                }
            }
            ...
            if (response.data?.choices?.[0]?.message?.content) {
                ...
                this.log.debug(`[API] Tool Server response received with ${contextItems} context items`);
                this.log.debug(`[API] Response content: ${content.substring(0, 200)}${content.length > 200 ? '...' : ''}`);
        ...
        try {
            ...
            try {
                ...
                if (result) {
                    ...
                    adapter.log.info(`[API] Response set for model ${modelName}: ${result.substring(0, 100)}${result.length > 100 ? '...' : ''}`);
                } else {
    ...
    async generateResponse(prompt, options = {}) {
        try {
            ...
            this.log.debug(`[OllamaClient] Generating response for prompt length: ${prompt.length}`);
            ...
            if (response.data?.choices?.[0]?.message?.content) {
                ...
                this.log.debug(`[OllamaClient] Generated response length: ${result.length}`);
                return result;
            } else {
                throw new Error('Invalid response format from OpenWebUI');
            }
        } catch (error) {
            this.log.error(`[OllamaClient] Generate response error: ${error.message}`);
            throw error;
        }
    }
