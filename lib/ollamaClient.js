...
class OllamaClient {
     ...
     */
    constructor(openWebUIUrl, ollamaUrl, logger, apiKey, setStateCallback, toolServerUrl) {
    ...
    /**
     * Process chat message via Tool Server with automatic RAG integration
     */
    async processChatMessage(modelName, messageObj, options = {}) {
        ...
        try {
            ...
            if (toolServerAvailable) {
                ...
                // Fallback to direct OpenWebUI (without RAG)
                this.log.warn(`[API] Tool Server not available - using direct OpenWebUI (no RAG)`);
                return await this._processChatViaDirect(modelName, messageObj, options);
            }
    ...
    /**
     * Process chat directly via OpenWebUI (fallback without RAG)
     */
    async _processChatViaDirect(modelName, messageObj, options = {}) {
        try {
            ...
            if (response && response.choices && response.choices[0]) {
                ...
                this.log.debug(`[API] Received direct response: ${answer.substring(0, 100)}...`);
