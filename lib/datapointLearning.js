...
class DatapointLearning {
  /**
   * Create a new DatapointLearning instance
   *
  ...
  constructor(adapter, log) {
    ...
    this.learningDataFile = path.join(
      adapterTempDir,
      `datapoint_learning_${this.adapter.namespace || "default"}.json`,
    );
  ...
  loadLearningData() {
    try {
      if (fs.existsSync(this.learningDataFile)) {
        ...
        this.log.debug(
          `[Learning] Loaded learning data with ${Object.keys(this.learningData.associations).length} associations`,
        );
      }
    } catch (error) {
  ...
  startPeriodicSaving() {
    if (this.saveTimer) {
      this.adapter.clearInterval(this.saveTimer);
    }

    // Only save if there are pending changes
    this.saveTimer = this.adapter.setInterval(() => {
  ...
  recordDatapointActions(datapointActions) {
    ...
    for (const dpAction of datapointActions) {
      ...
      this.log.debug(
        `[Learning] Recorded action for ${dpAction.id}: ${dpAction.value}`,
      );
    }
  ...
  recordAssociation(primaryId, partnerId, context) {
    ...
    associations.partners[partnerId] =
      (associations.partners[partnerId] || 0) + 1;
    ...
    if (context) {
      ...
      associations.contexts[partnerId][context] =
        (associations.contexts[partnerId][context] || 0) + 1;
    }
  ...
  createSceneFromAssociations(mainDatapointId, sceneName) {
    ...
    const sceneDatapoints = [
      mainDatapointId,
      ...partners.map((p) => p.datapointId),
    ];
    const scene = {
      name: sceneName,
    ...
    this.log.info(
      `[Learning] Created scene "${sceneName}" with ${sceneDatapoints.length} datapoints`,
    );
   ...
   */
  getLearnedScenes() {
    return Object.values(this.learningData.scenes).sort(
  ...
  suggestScenes(minAssociations = 2) {
    ...
    for (const [datapointId] of Object.entries(
      this.learningData.associations,
    )) {
      const partners = this.getSuggestedPartners(datapointId);

      if (partners.length >= minAssociations) {
        // Check if scene already exists
        const existingScene = Object.values(this.learningData.scenes).find(
          (scene) => scene.datapoints.includes(datapointId),
        );
  ...
  generateSceneName(mainDatapoint, partners) {
    ...
    const partnerNames = partners
      .slice(0, 2)
      .map((p) => extractName(p.datapointId));
  ...
  getStatistics() {
    ...
    const totalAssociations = Object.values(
      this.learningData.associations,
