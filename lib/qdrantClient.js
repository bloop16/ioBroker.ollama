...
class QdrantHelper {
  ...
  static async processEmbeddingDatapoint(
    ...
    try {
      ...
      // Simple deduplication using cache interface (Set or LRUCache)
      const stateKey = `${id}_${state.val}_${Math.floor(state.ts / 60000)}`;
      if (processedStates.has && processedStates.has(stateKey)) {
        return false;
      }

      if (processedStates.add) {
        processedStates.add(stateKey);
      } else if (processedStates.set) {
        processedStates.set(stateKey, true);
      }

      // Cleanup old entries periodically (only for Set-based cache)
      if (
        processedStates.size > 500 &&
