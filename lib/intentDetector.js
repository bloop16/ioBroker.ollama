...
class IntentDetector {
    constructor(log = console, translateFn = (key) => key) {
        ...
        this.controlPatterns = {
            de: {
                keywords: ['einschalten', 'ausschalten', 'setzen', 'ändern', 'schalten', 'umschalten', 'anpassen', 'stellen', 'machen', 'aktivieren', 'deaktivieren', 'ist', 'ankommen', 'verlassen', 'ein', 'aus', 'an'],
                patterns: [
            ...
            en: {
                keywords: ['turn', 'set', 'change', 'switch', 'toggle', 'adjust', 'activate', 'deactivate', 'make', 'enable', 'disable', 'is', 'arrived', 'left', 'on', 'off'],
                patterns: [
                    /turn\s+(.+?)\s+(on|off)/i,
                    /switch\s+(.+?)\s+(on|off)/i,
                    /make\s+(.+?)\s+(on|off)/i,
        ...
        this.log.info(this.translate('general_initialized').replace('{{component}}', 'IntentDetector'));
    }
    ...
    detectControlIntent(text) {
        ...
        const languagesToCheck = [detectedLanguage, ...Object.keys(this.controlPatterns)]
    ...
    detectLanguage(text) {
        ...
        const germanIndicators = ['ist', 'und', 'der', 'die', 'das', 'ein', 'eine', 'auf', 'mit', 'von', 'zu', 'in', 'an', 'für', 'setze', 'schalte'];
        const englishIndicators = ['is', 'and', 'the', 'a', 'an', 'to', 'with', 'of', 'in', 'on', 'at', 'for', 'set', 'turn'];
    ...
    _extractActionFromPattern(match, pattern, lang) {
        ...
        if (patternStr.includes('(ein|aus|an)') || patternStr.includes('(on|off)')) {
            const actionPart = match[2] || match[3];
            const isOn = actionPart && (actionPart.includes('on') || actionPart.includes('ein') || actionPart.includes('an'));
            return {
                language: lang,
        ...
        if (patternStr.includes('to\\s+(.+)') || patternStr.includes('auf\\s+(.+)')) {
        ...
        if (patternStr.includes('(da|zuhause|angekommen|anwesend|verfügbar)') || 
        ...
        if (patternStr.includes('(weg|nicht\\s+da|abwesend|nicht\\s+zuhause)') ||
    ...
    _inferActionFromKeywords(text, keywords) {
        if (text.includes('on') || text.includes('ein') || text.includes('an') || 
            text.includes('activate') || text.includes('aktiviere')) {
            return this.actionTypes.ON;
        }
        
        if (text.includes('off') || text.includes('aus') || 
            text.includes('deactivate') || text.includes('deaktiviere')) {
            return this.actionTypes.OFF;
        }
        
        if (text.includes('toggle') || text.includes('umschalten') || text.includes('schalten')) {
    ...
    _extractTarget(text, keywords) {
        ...
        const commonWords = ['the', 'a', 'an', 'to', 'on', 'off', 'at', 'in', 'with',
                             'der', 'die', 'das', 'ein', 'eine', 'auf', 'an', 'aus', 'zu'];
    ...
    isSceneAction(text) {
        const sceneIndicators = [
            'alle', 'mehrere', 'scene', 'szene', 'gruppe', 'bereich',
            'all', 'multiple', 'scene', 'group', 'room', 'area',
            'and', 'und', 'with', 'mit'
        ];

        const lowerText = text.toLowerCase();
        const hasSceneIndicator = sceneIndicators.some(indicator => lowerText.includes(indicator));
