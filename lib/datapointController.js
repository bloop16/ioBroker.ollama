...
class DatapointController {
    constructor(adapter, allowedDatapoints = new Set(), log = console, translateFn = (key) => key) {
        this.adapter = adapter;
        this.allowedDatapoints = allowedDatapoints;
        this.log = log;
        this.translate = translateFn;
        
        this.log.info(this.translate('general_initialized').replace('{{component}}', 'DatapointController'));
    }
    ...
    async getDatapointValue(datapointId) {
        try {
            ...
            this.log.error(`[DatapointController] Error getting value for ${datapointId}: ${error.message}`);
            return {
                success: false,
                error: error.message,
                datapointId: datapointId
    ...
    async setDatapoint(datapointId, value) {
        try {
            ...
            if (this.allowedDatapoints.size > 0 && !this.allowedDatapoints.has(datapointId)) {
                this.log.warn(this.translate('datapoint_not_allowed').replace('{{datapoint}}', datapointId));
                return false;
            }
            ...
            if (!obj || !obj.common) {
                this.log.error(this.translate('general_error_occurred').replace('{{error}}', `Datapoint ${datapointId} not found`));
                return false;
            }
            ...
            if (convertedValue === null) {
                this.log.error(this.translate('datapoint_type_conversion_failed')
                    .replace('{{value}}', value)
            ...
            this.log.info(this.translate('datapoint_set_success')
                .replace('{{datapoint}}', datapointId)
                .replace('{{value}}', convertedValue));
            return true;

        } catch (error) {
            this.log.error(this.translate('datapoint_set_failed')
                .replace('{{datapoint}}', datapointId)
    ...
    convertValue(value, targetType, customConfig = {}) {
        try {
            ...
            this.log.warn(this.translate('datapoint_type_conversion_failed')
                .replace('{{value}}', value)
    ...
    convertToBoolean(value, customConfig) {
        ...
        if (typeof value === 'string') {
            ...
            const presenceTrue = [
                // English
                'true', 'on', '1', 'yes', 'home', 'present', 'available', 'here', 'arrived',
                // German
                'wahr', 'ein', 'an', 'ja', 'zuhause', 'anwesend', 'verfügbar', 'da', 'angekommen',
                // French
                'vrai', 'allumé', 'oui', 'maison', 'présent', 'disponible', 'ici', 'arrivé',
                // Spanish
                'verdadero', 'encendido', 'sí', 'casa', 'presente', 'disponible', 'aquí', 'llegado'
            ];
            
            const presenceFalse = [
                // English
                'false', 'off', '0', 'no', 'away', 'absent', 'gone', 'left',
                // German
                'falsch', 'aus', 'ab', 'nein', 'weg', 'abwesend', 'fort', 'verlassen',
                // French
                'faux', 'éteint', 'non', 'absent', 'parti', 'quitté',
                // Spanish
                'falso', 'apagado', 'no', 'ausente', 'ido', 'salido'
            ];
    ...
    async handleFunctionCall(functionName, args) {
        switch (functionName) {
            ...
            default:
                return {
                    success: false,
                    message: this.translate('general_not_available').replace('{{feature}}', functionName)
                };
        }
    }
}
