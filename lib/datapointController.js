  ...
  async _resolveWithVectorDatabase(inputDatapoint) {
    try {
      ...
      for (const match of searchResults) {
        ...
        if (!datapointId || !this.allowedDatapoints.has(datapointId)) {
          continue;
        }
...
Do NOT convert values - use them exactly as the datapoint expects. Available datapoints: ${datapointList}`,
  ...
  async executeSetStateDirectly(parameters) {
    ...
    try {
      ...
      if (!this.writeAllowedDatapoints.has(resolvedDatapoint)) {
        return {
          success: false,
  ...
  async executeSetState(parameters) {
    ...
    try {
      ...
      // Setting failed - provide basic error information
      this.log.error(
        `[DatapointController] Failed to set ${resolvedDatapoint} - check permissions and configuration`,
      );
