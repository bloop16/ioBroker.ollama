...
class ConfigValidator {
  ...
  validateVectorDb(config) {
    ...
    if (!config.vectorDbCollection || config.vectorDbCollection.trim() === "") {
      warnings.push(
        "Vector collection name not specified, using default 'iobroker_datapoints'",
      );
      sanitized.vectorDbCollection = "iobroker_datapoints";
    } else {
      sanitized.vectorDbCollection = config.vectorDbCollection.trim();
    }
