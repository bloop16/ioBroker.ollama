...
class ToolServer {
      ...
      if (
        ...
        try {
          ...
          if (intent.value !== undefined) {
            result = await this.datapointController.setDatapointValue(
              intent.target,
              intent.value,
            );
  ...
  async _processControlIntent(intent, originalQuery) {
    try {
      ...
      // Execute the control action
      const result = await this.datapointController.setDatapointValue(
        datapointId,
        intent.value,
      );
    ...
    try {
      ...
      if (result.success) {
        ...
        if (datapointId && this.datapointController) {
          const forceResult = await this.datapointController.setDatapointValue(
            datapointId,
            value,
          );
          res.json({
