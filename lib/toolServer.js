...
class ToolServer {
  ...
  _setupRoutes() {
    ...
    this.app.post("/chat/completions", async (req, res) => {
      try {
        ...
        this.log.debug(
          `[ToolServer] Processing chat completion for model: ${model}`,
        );
    ...
    this.app.get("/tools", (req, res) => {
      try {
        const tools = this.datapointController
          ? this.datapointController.getFunctionDefinitions()
          : [];
        res.json(tools);
      } catch (error) {
    ...
    this.app.post("/tools/ai-control", async (req, res) => {
      try {
        ...
        this.log.error(
          `[ToolServer] Error in AI control endpoint: ${error.message}`,
        );
        res.status(500).json({ error: "Internal server error" });
      }
    });
    ...
    this.app.post("/debug/test-intent", async (req, res) => {
      try {
        ...
        this.log.error(
          `[ToolServer] Error in test-intent endpoint: ${error.message}`,
        );
        res.status(500).json({ error: "Internal server error" });
      }
    });
    ...
    this.app.get("/debug/ai-control-status", async (req, res) => {
      try {
        ...
        this.log.error(
          `[ToolServer] Error in ai-control-status endpoint: ${error.message}`,
        );
        res.status(500).json({ error: "Internal server error" });
      }
    });
    ...
    this.app.post("/ai-control/execute", async (req, res) => {
      try {
        ...
        this.log.error(
          `[ToolServer] Error in direct AI control endpoint: ${error.message}`,
        );
        res.status(500).json({ error: "Internal server error" });
      }
    });
    ...
    if (this.learningSystem) {
      ...
      this.app.get("/learning/suggestions/:datapointId", (req, res) => {
        try {
          if (!this.learningSystem) {
            return res
              .status(503)
              .json({ error: "Learning system not available" });
          }
          const { datapointId } = req.params;
          const { minFrequency = 1 } = req.query;
          const suggestions = this.learningSystem.getSuggestedPartners(
            datapointId,
          ...
          this.log.error(
            `[ToolServer] Error getting suggestions: ${error.message}`,
          );
      ...
      this.app.post("/learning/scenes/:sceneName", (req, res) => {
        try {
          ...
          res.json({
            message: "Scene execution would be implemented here",
            scene,
          });
        } catch (error) {
          this.log.error(
            `[ToolServer] Error executing scene: ${error.message}`,
          );
  ...
  _getOpenAPISpec() {
    return {
      ...
      info: {
        ...
        description:
          "Query ioBroker datapoints using RAG (Retrieval Augmented Generation)",
        version: "1.0.0",
      },
      servers: [
        {
      ...
      paths: {
        "/tools/iobroker-rag": {
          post: {
            ...
            description:
              "Use RAG to query ioBroker datapoints and get context-aware responses",
                  ...
                  schema: {
                    ...
                    properties: {
                      query: {
                        type: "string",
                        description:
                          "The query to search for in ioBroker datapoints",
                      },
                    },
                    required: ["query"],
                  },
                    ...
                    schema: {
                      ...
                      properties: {
                        answer: {
                          type: "string",
                          description:
                            "The RAG response with datapoint information",
                        },
                        context_used: {
                          type: "array",
   ...
   *
   * @param {object} chatRequest - The chat completion request object
   */
    ...
    try {
      ...
      if (this.intentDetector) {
        ...
        if (intent.isControl && intent.confidence > 0.5) {
          ...
          try {
            this.log.debug(
              `[ToolServer] Processing control intent with value: ${intent.value} (type: ${typeof intent.value})`,
            );
            const controlResult = await this._processControlIntent(
              intent,
              userQuery,
            );
            ...
            this.log.warn(
              `[ToolServer] Control intent failed, falling back to RAG. Error: ${controlResult.error}`,
            );
          } catch (controlError) {
            this.log.error(
              `[ToolServer] Control processing error: ${controlError.message}`,
            );
            this.log.error(
              `[ToolServer] Control error stack:`,
              controlError.stack,
            );
            // Fall through to RAG
          }
          ...
          this.log.debug(
            `[ToolServer] Intent not recognized as control (isControl: ${intent.isControl}, confidence: ${intent.confidence})`,
          );
        }
      }
      ...
      if (use_rag && this.qdrantClient) {
        try {
          ...
          const contextResults = await this._searchSimilarDatapoints(
            queryEmbedding,
            5,
          );
          ...
          if (contextResults.length > 0) {
            ...
            this.log.debug(
              `[ToolServer] Enhanced query with ${contextResults.length} context items`,
            );
          }
        } catch (ragError) {
          this.log.warn(
            `[ToolServer] RAG enhancement failed: ${ragError.message}`,
          );
      ...
      const chatResponse = await this._generateChatResponse(
        model,
        finalMessages,
        temperature,
        max_tokens,
      ...
      this.log.error(
        `[ToolServer] Complete chat processing error: ${error.message}`,
      );
      throw error;
    }
  }
  ...
  async _generateChatResponse(
    ...
    try {
      ...
      try {
        ...
        this.log.debug(
          `[ToolServer] OpenWebUI chat failed, trying Ollama direct: ${openWebUIError.message}`,
        );
      ...
      this.log.error(
        `[ToolServer] Chat response generation failed: ${error.message}`,
      );
  ...
  async _generateEmbedding(text) {
    try {
      ...
      const response = await axios.post(
        `${this.openWebUIUrl}/ollama/api/embed`,
        {
      ...
      this.log.error(
        `[ToolServer] Embedding generation failed: ${error.message}`,
      );
  ...
  async _searchSimilarDatapoints(queryEmbedding, limit = 5) {
    try {
      ...
      this.log.debug(
        `[ToolServer] Found ${searchResult.length} similar datapoints`,
      );
      return searchResult;
    } catch (error) {
   ...
   *
   * @param {Array} results - Array of search results from vector database
   */
   ...
   * @param {string} context - The context information for the answer
   */
  async _generateContextualAnswer(query, context) {
    try {
      const systemPrompt =
        "You are a helpful smart home assistant. Use the provided context to answer questions about the smart home status. ALWAYS use the most recent data (marked as 'MOST RECENT') when available. Answer in the same language as the question.";
  ...
  async _handleAIControlRequest(req, res) {
    try {
      ...
      if (
        ...
        try {
          ...
          if (intent.value !== undefined) {
            result = await this.datapointController.setDatapoint(
              intent.target,
              intent.value,
            );
            currentDatapoints.push({ id: intent.target, value: intent.value });
            this.log.info(
              `[ToolServer] AI Control - Set ${intent.target} to ${intent.value}`,
            );
          } else {
          ...
          if (this.learningSystem) {
            ...
            if (intent.isScene && intent.sceneName) {
              this.learningSystem.createSceneFromAssociations(
                intent.target,
                intent.sceneName,
              );
            }
          ...
          this.log.error(
            `[ToolServer] Control action failed: ${controlError.message}`,
          );
      ...
      this.log.error(
        `[ToolServer] AI control request processing failed: ${error.message}`,
      );
      res.status(500).json({ error: "Internal server error" });
    }
  }
   ...
   *
   * @param {object} intent - The detected intent object
  ...
  async _processControlIntent(intent, originalQuery) {
    try {
      ...
      if (
        !this.datapointController ||
        !this.datapointController.allowedDatapoints.has(datapointId)
      ) {
        return {
          success: false,
      ...
      // Execute the control action
      const result = await this.datapointController.setDatapoint(
        datapointId,
        intent.value,
      );
      ...
      this.log.error(
        `[ToolServer] Control intent processing error: ${error.message}`,
      );
      return {
        success: false,
        error: error.message,
   ...
   *
   * @param {string} target - The target description to match against
   */
   ...
   *
   * @param {string} target - The target description to match
   * @param {string} datapointId - The datapoint ID to match against
   */
      ...
      for (const dpWord of datapointWords) {
        if (dpWord.includes(targetWord) || targetWord.includes(dpWord)) {
          if (dpWord === targetWord) {
            ...
            dpWord.includes(targetWord) ||
            targetWord.includes(dpWord)
          ) {
  ...
  async _findAvailablePort(startPort = this.port) {
    for (
      let port = startPort;
      port < startPort + this.maxPortAttempts;
      port++
    ) {
    ...
    throw new Error(
      `No available port found in range ${startPort}-${startPort + this.maxPortAttempts}`,
    );
  }
  ...
  async start() {
    ...
    try {
      ...
      if (await this.controller.isRunning()) {
        ...
        this.log.warn(
          `[ToolServer] Another Tool Server is already running on port ${runningInstance?.port || "unknown"}`,
        );
      ...
      if (!qdrantConnected) {
        this.log.warn(
          "[ToolServer] Starting without Qdrant connection - RAG queries will fail",
        );
      }
      ...
      return new Promise((resolve, reject) => {
        const server = this.app.listen(this.port, this.host, (error) => {
          if (error) {
            this.log.error(
              `[ToolServer] Failed to start server: ${error.message}`,
            );
            reject(error);
          } else {
            this.server = server;
            ...
            this.log.info(
              `[ToolServer] Started on http://${this.host}:${this.port}`,
            );
            this.log.info(
              `[ToolServer] OpenAPI spec: http://${this.host}:${this.port}/openapi.json`,
            );
            this.log.info(
              `[ToolServer] RAG endpoint: http://${this.host}:${this.port}/tools/iobroker-rag`,
            );
            resolve(true);
          }
        });
  ...
  async _handleTestIntent(req, res) {
    try {
      ...
      const intent = await this.intentDetector.detectControlIntent(query);
      const bestDatapoint = intent.isControl
        ? await this._findBestMatchingDatapoint(intent.target)
        : null;
      ...
      const response = {
        ...
        allowedForControl: bestDatapoint
          ? this.datapointController &&
            this.datapointController.allowedDatapoints.has(bestDatapoint)
          : false,
        debug: {
          intentDetectorAvailable: !!this.intentDetector,
          datapointControllerAvailable: !!this.datapointController,
          allowedDatapointsCount: this.datapointController
            ? this.datapointController.allowedDatapoints.size
            : 0,
        },
      };
    ...
    try {
      ...
      const response = {
        ...
        config: {
          ...
          intentDetectorLanguages: this.intentDetector
            ? ["de", "en", "fr", "es"]
            : [],
        },
      };
  ...
  async _handleDirectAIControl(req, res) {
    try {
      ...
      if (!query && (!target || value === undefined)) {
        return res.status(400).json({
          error:
            "Either 'query' or both 'target' and 'value' parameters are required",
        });
      }
      ...
      if (query) {
        ...
        if (!this.intentDetector) {
          return res
            .status(503)
            .json({ error: "Intent detector not available" });
        }
      ...
      const result = await this._processControlIntent(
        intent,
        query || `Direct control: ${target} = ${value}`,
      );
      ...
      if (result.success) {
        ...
        if (datapointId && this.datapointController) {
          const forceResult = await this.datapointController.setDatapoint(
            datapointId,
            value,
          );
          res.json({
            ...
            message: forceResult
              ...
              : `Force execution failed: Operation failed`,
            datapoint: datapointId,
            value: value,
            forced: true,
            intent: intent,
          ...
          res.json({
            success: false,
            message:
              result.error || "No matching datapoint found for force execution",
            intent: intent,
          });
        }
      } else {
