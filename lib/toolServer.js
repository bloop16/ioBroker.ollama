...
class ToolServer {
    ...
    try {
      ...
      if (response.data?.data) {
        ...
        if (!isAvailable) {
          ...
          this.log.info(
            `[ToolServer] Hint: Check if model "${model}" is properly configured in OpenWebUI at ${this.openWebUIUrl}`,
          );
        } else {
          this.log.debug(
            `[ToolServer] Model "${model}" confirmed available in OpenWebUI`,
          );
        }
      ...
      this.log.warn(
        `[ToolServer] Invalid response format from OpenWebUI models API`,
      );
      return false;
    } catch (error) {
      this.log.error(
      ...
      this.log.error(
        `[ToolServer] OpenWebUI URL: ${this.openWebUIUrl}/api/models`,
      );
      return false; // Changed to false - fail safely if we can't verify
    }
  }
  ...
  async _callOpenWebUI(model, messages, temperature, maxTokens, tools = []) {
    ...
    try {
      ...
      // Handle both response.choices and response.data.choices formats
      const choices = response.choices || response.data?.choices;

      // Handle function calls if present
      if (choices?.[0]?.message?.tool_calls) {
        this.log.debug(
          `[ToolServer] Function calls detected: ${choices[0].message.tool_calls.length}`,
        );
      ...
      if (!content) {
        ...
        this.log.debug(
          `[ToolServer] Response data structure:`,
          JSON.stringify(
            {
        ...
        // Check if model is actually available in OpenWebUI
        const modelAvailable = await this._isModelAvailable(model);
        if (!modelAvailable) {
          const errorMsg = `Model "${model}" is not available in OpenWebUI. Please check your OpenWebUI configuration at ${this.openWebUIUrl}`;
          this.log.error(`[ToolServer] ${errorMsg}`);
          return errorMsg;
        }
      ...
      // Log additional error details safely (avoid circular reference)
      if (error.response) {
        this.log.debug(`[ToolServer] HTTP Status: ${error.response.status}`);
        this.log.debug(
          `[ToolServer] HTTP Status Text: ${error.response.statusText}`,
        );
        if (error.response.data) {
          this.log.debug(
            `[ToolServer] Error Response Data:`,
            JSON.stringify(error.response.data, null, 2),
          );
        }
      }

      throw new Error(`OpenWebUI API request failed: ${error.message}`);
    }
  }
  ...
  async _handleFunctionCalls(
    ...
    try {
      ...
      const finalChoices = finalResponse.choices || finalResponse.data?.choices;
      return (
        finalChoices?.[0]?.message?.content ||
