...
class ToolServer {
  ...
  _startProgressMonitor(requestType, model) {
    ...
    const intervalId = setInterval(() => {
      ...
      this.log.info(
        `[ToolServer ${requestType}] Request with model "${model}" is still running (${timeStr}, check #${checkCount})`,
      );
    }, 30000); // Every 30 seconds

    this.log.info(
      `[ToolServer ${requestType}] Started request with model "${model}"`,
    );
   ...
   *
   * @param {number} intervalId - Interval ID from _startProgressMonitor
   * @param {string} requestType - Type of request
  ...
  _stopProgressMonitor(intervalId, requestType, model, success = true) {
    if (intervalId) {
      ...
      this.log.info(
        `[ToolServer ${requestType}] Request with model "${model}" ${status}`,
      );
    }
  }
  ...
  _getOpenAPISpec() {
    ...
    spec.servers[0].url = `http://${this.host === "0.0.0.0" ? "localhost" : this.host}:${this.port}`;
    ...
    if (allowedDatapoints.length > 0) {
      // Add enum to setState datapoint parameter
      const setStateDatapoint =
        spec.paths["/setState"]?.post?.requestBody?.content?.[
          "application/json"
        ]?.schema?.properties?.datapoint;
      if (setStateDatapoint) {
        setStateDatapoint["enum"] = allowedDatapoints;
      }

      // Add enum to getState datapoint parameter
      const getStateDatapoint =
        spec.paths["/getState"]?.post?.requestBody?.content?.[
          "application/json"
        ]?.schema?.properties?.datapoint;
      if (getStateDatapoint) {
        getStateDatapoint["enum"] = allowedDatapoints;
      }
    }
  ...
  async _callOpenWebUI(model, messages, temperature, maxTokens, tools = []) {
    ...
    try {
      ...
      progressMonitor = this._startProgressMonitor("Chat Completion", model);
      ...
      if (functionCallMatch) {
        ...
        try {
          ...
          this.log.error(
            `[ToolServer] Text-based function call error: ${error.message}`,
          );
  ...
  async _handleFunctionCalls(
    ...
    for (const toolCall of toolCalls) {
      try {
        ...
        this.log.error(
          `[ToolServer] Function execution error: ${error.message}`,
        );
  ...
  async _initializeQdrant() {
    ...
    try {
      this.qdrantClient = new QdrantClient({
        url: `http://${this.config.vectorDbIp}:${this.config.vectorDbPort}`,
      });
