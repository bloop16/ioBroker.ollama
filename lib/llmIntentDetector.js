...
class LLMIntentDetector {
    constructor(ollamaClient, log = console, translateFn = (key) => key) {
        ...
        this.log.info(this.translate('general_initialized').replace('{{component}}', 'LLMIntentDetector'));
    }

    /**
     * Detect control intent using LLM
     * @param {string} text - User input
     * @param {Array} availableDatapoints - Context for better matching
    ...
    async detectControlIntent(text, availableDatapoints = []) {
        try {
            ...
            this.log.error(this.translate('llm_error').replace('{{error}}', error.message));
    ...
    async isAvailable() {
        try {
            ...
            this.log.debug(this.translate('llm_unavailable').replace('{{error}}', error.message));
            return false;
        }
    }
