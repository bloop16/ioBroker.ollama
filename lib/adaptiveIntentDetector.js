...
class AdaptiveIntentDetector {
  /**
   * Create a new AdaptiveIntentDetector instance
   *
   * @param {object} ollamaClient - OllamaClient instance for LLM-based detection
  ...
  constructor(ollamaClient = null, log = console, translateFn = (key) => key) {
    ...
    this.llmDetector = ollamaClient
      ? new LLMIntentDetector(ollamaClient, log, translateFn)
      : null;
  ...
  async detectControlIntent(text, availableDatapoints = []) {
    try {
      ...
      if (this.config.useLLM && this.llmDetector) {
        try {
          this.log.debug(
            `[AdaptiveIntentDetector] Trying LLM detection for: "${text}"`,
          );
          result = await this.llmDetector.detectControlIntentWithContext(
            text,
            availableDatapoints,
          );
          ...
          if (
            ...
            this.log.debug(
              `[AdaptiveIntentDetector] LLM detection successful: confidence ${result.confidence}`,
            );
            result.detectionMethod = "LLM";
            return result;
          }
          this.log.debug(
            `[AdaptiveIntentDetector] LLM confidence too low: ${result?.confidence || 0}`,
          );
        } catch (llmError) {
          this.log.warn(
            `[AdaptiveIntentDetector] LLM detection failed: ${llmError.message}`,
          );
        }
      }
      ...
      if (this.config.fallbackToPattern) {
        this.log.debug(
          `[AdaptiveIntentDetector] Trying pattern detection for: "${text}"`,
        );
        result = this.patternDetector.detectControlIntent(text);

        if (
          result &&
          result.confidence >= this.config.patternConfidenceThreshold
        ) {
          this.log.debug(
            `[AdaptiveIntentDetector] Pattern detection successful: confidence ${result.confidence}`,
          );
          result.detectionMethod = "Pattern";
          return result;
        }
        this.log.debug(
          `[AdaptiveIntentDetector] Pattern confidence too low: ${result?.confidence || 0}`,
        );
      }

      // No detection method succeeded
      this.log.debug(
        `[AdaptiveIntentDetector] No detection method succeeded for: "${text}"`,
      );
      return {
        isControl: false,
        confidence: 0,
      ...
      this.log.error(
        `[AdaptiveIntentDetector] Detection error: ${error.message}`,
      );
      return {
        isControl: false,
        confidence: 0,
  ...
  updateConfig(newConfig) {
    ...
    this.log.info(
      `[AdaptiveIntentDetector] Configuration updated: ${JSON.stringify(this.config)}`,
    );
  }
   ...
   * @returns {Array} Array of scene targets found in the text
   */
